...
from game_dev_idle.app.auth.pkce import get_pkce_challenge_pair
from game_dev_idle.app.auth.state import generate_random_state
...
class URL:
    ...
    def include_query_params(self, **kwargs: typing.Any) -> typing.Self:
        params = MultiDict(parse_qsl(self.query, keep_blank_values=True))
        params.update({str(key): str(value) for key, value in kwargs.items()})
        query = urlencode(params.multi_items())
        return self.replace(query=query)

    def replace_query_params(self, **kwargs: typing.Any) -> typing.Self:
        query = urlencode([(str(key), str(value)) for key, value in kwargs.items()])
        return self.replace(query=query)

    def remove_query_params(self, keys: str | typing.Sequence[str]) -> typing.Self:
...
class SSOBase:
    ...
    async def get_login_redirect(
        ...
        *,
        ...
        response = Redirect(login_uri, status_code=303)
        if self.uses_pkce:
    ...
    async def verify_and_process(
        ...
        *,
        ...
        Args:
            ...
            state (Optional[str]): The state parameter returned from the login request.
            pkce_code_verifier (Optional[str]): The PKCE code verifier used in the login request.
            url (URL): The full URL of the callback endpoint.
